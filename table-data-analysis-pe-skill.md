...
**核心原则**：文件解析 → 目的识别 → 指标/维度识别 → 查询企业知识 → 建立分析体系 → **分析方法说明与统计学推断** → 异动检测 → 计算校验 → 结构化输出
...
digraph table_analysis_flow {
    ...
    subgraph cluster_4_5 {
        label="第五步（1/2）：分析方法说明与统计学推断";
        explain_methods [label="分析方法说明\n（先告知用户用什么方法）" shape=box style=filled fillcolor=lightcyan];
        infer_purpose [label="从问题推断分析目的\n和潜在意图" shape=box style=filled fillcolor=lightcyan];
        statistical_inference [label="统计学推断\n（数据论证，好/坏有依据）" shape=box style=filled fillcolor=lightcyan];
    }
    ...
    define_system -> explain_methods;
    explain_methods -> infer_purpose;
    infer_purpose -> statistical_inference;
    statistical_inference -> check_deviation;
...
## 第五步（1/2）：分析方法说明与统计学推断

**核心目标**：在输出计算结果前，先向用户说明"用什么方法分析"和"为什么选这个方法"，结论要有统计学依据，不能直接说"好/坏"。

---

### 5.1.1 分析方法说明

**必须先于计算结果输出**。告诉用户本次分析选用了哪些方法，以及为什么选用。

#### 分析方法库（常用）

| 方法类别 | 具体方法 | 适用场景 | 统计依据 |
|---------|---------|---------|---------|
| **增速衡量** | 环比增长率 | 衡量相邻周期变化 | (本期-上期)/上期 × 100% |
| **增速衡量** | 同比增长率 | 衡量同期年度变化 | (本期-去年同期)/去年同期 × 100% |
| **增速衡量** | 复合增长率(CAGR) | 衡量多年平均增速 | (期末/期初)^(1/n)-1 |
| **贡献度衡量** | 增量贡献度 | 衡量绝对增量的相对贡献 | 某增量/总增量 × 100% |
| **贡献度衡量** | 增量贡献排名 | 对比各维度的绝对贡献 | 排序 |
| **偏离度衡量** | 增幅偏离均值 | 识别统计异常 | (增幅-均值)/标准差 |
| **偏离度衡量** | Z-Score | 标准化比较不同量级 | (x-μ)/σ |
| **偏离度衡量** | 变异系数(CV) | 衡量波动稳定性 | 标准差/均值 |
| **分层对比** | 量级分层对比 | 不同基数城市对比 | 按基数分段后再比 |
| **分层对比** | 基准对比 | 与整体均值/目标值对比 | 计算偏离度 |
| **趋势分析** | 趋势拟合 | 识别长期趋势 | 线性回归/多项式拟合 |
| **趋势分析** | 环比序列 | 观察连续变化 | 序列图 |

#### 方法选择逻辑

| 用户问题特征 | 推荐首选方法 | 补充方法 |
|-------------|------------|---------|
| "哪个城市增幅最快" | 环比增长率 + 增量贡献度 | 增幅偏离度、量级分层 |
| "哪个城市表现最好" | 增量贡献度 + 增幅偏离度 | 趋势稳定性、量级权重 |
| "为什么下降" | 贡献度分析 + 维度拆分 | 增量分解、异动定位 |
| "趋势如何" | 环比序列 + 趋势拟合 | 季节性分解 |
| "对比A和B" | 增长率对比 + 基准对比 | 统计显著性检验 |
...
**核心原则**：在给出"好/坏"结论之前，先进行统计学推断，给出数据论证过程。

#### 常见统计学陷阱及应对

| 陷阱 | 问题 | 正确做法 |
|-----|------|---------|
| **基数效应** | 无锡10万+50%=5万增量 vs 常州1万+100%=1万增量 | 不能直接比增幅%，要比增量绝对值 |
| **增长率下降≠负增长** | 增速从50%降到20% | 仍在增长，只是增速放缓，需结合绝对值判断 |
| **平均数陷阱** | 各城市平均增速30% | 平均值掩盖了个体差异，需看分布 |
| **辛普森悖论** | 分组看和整体看结论相反 | 需要同时看分组和整体 |

#### 统计学推断示例

**场景**：无锡10万量级+50%增幅 vs 常州1万量级+100%增幅，哪个表现更好？

```markdown
### 统计学推断

**问题**：两个城市增幅不同量级，如何判断"表现更好"？

**推断过程**：

① **基数效应分析**：
| 城市 | 基数(上期) | 增幅 | 绝对增量 | 计算过程 |
|-----|-----------|------|---------|---------|
| 无锡 | 10万 | 50% | 5万 | 10万 × 50% |
| 常州 | 1万 | 100% | 1万 | 1万 × 100% |

→ **推断**：无锡的绝对增量(5万)是常州的5倍，贡献更大。

② **增幅偏离度分析**（假设6个城市平均增幅为40%，标准差为25%）：
| 城市 | 实际增幅 | 均值偏离 | Z-Score | 判断 |
|-----|---------|---------|---------|------|
| 无锡 | 50% | +10% | +0.4σ | 在1σ内，正常波动 |
| 常州 | 100% | +60% | +2.4σ | 超过2σ，属于统计异常 |

→ **推断**：常州增幅远超平均，可能存在基数低导致的"虚高"。

③ **量级权重考量**：
| 考量维度 | 无锡 | 常州 |
|---------|------|------|
| 基数权重 | 大基数，稳定性强 | 小基数，波动性大 |
| 增量贡献 | 贡献总增量83% | 贡献总增量17% |
| 增长质量 | 高基数+高增幅，质量优 | 低基数+高增幅，质量待验证 |

**统计学推断结论**：
- 无锡：✅ 表现更优。（高基数(10万)下实现50%增幅，绝对增量5万，贡献了绝大多数增量，且增幅在统计合理范围内）
- 常州：⚠️ 增幅虽高(100%)，但基数极小(1万)，绝对增量有限(1万)，且100%增幅超出统计异常阈值，需关注增长质量。

**结论依据**：
- 不能直接说"常州增幅大所以更好"
- 必须结合：基数大小(影响稳定性) + 绝对增量(影响实际贡献) + 统计显著性(判断增幅是否合理)
```

---

### 5.1.3 方法说明与统计学推断输出模板
...
**输出原则：文件信息 → 指标识别 → 分析体系 → 分析方法说明 → 统计学推断 → 异动检测 → 结论+证据+计算过程**
...
[在给出"好/坏"结论之前，先进行统计学推断，给出数据论证过程]
...
| 10 | 没有分析体系 | 先建立框架再执行 |
| 11 | 不查企业知识 | 必须查询，说明结果 |
| 12 | 直接输出"好/坏"结论，无数据论证 | 分析方法说明 + 统计学推断，必须先论证再结论 |
| 13 | 增幅对比忽略基数效应 | 量级分层对比 + 增量贡献度分析 |
...
- "直接给结论" → **必须先输出分析方法说明和统计学推断**
- "增幅大的就是好" → **必须考虑基数效应，分析增量贡献度**
...
| 版本 | 日期 | 变更说明 |
|-----|------|---------|
| 0.4.0 | 2026-04-28 | **重构**：新增"第五步(1/2)：分析方法说明与统计学推断"，在计算结果前先说明分析方法论并给出统计学推断；新增生产问题#12、#13；更新流程图；更新结构化输出模板 |
